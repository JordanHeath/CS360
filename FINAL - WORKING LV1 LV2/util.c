...
int enter_name(MINODE *pip, int myino, char *myname)
{
	...
	for (i = 0; i < 13; i++)
	{
		...
		{
			//printf("FULL?\n");
			break;//break if we hsve nothing allocated
		}
...
int myread(int temp_fd, char buff[], int nbytes)
{
	...
	while (nbytes && avail)
	{
		...
		else
		{
			...
			lbk -=(12+256);
			blkhold = tempbuff2[(lbk)/256];

			get_block(dev, blkhold, tempbuff3);
			blk = tempbuff3[(lbk)%256];
	...
	while (nbytes > 0)
	{
		...
		else if (lbk >= 12 && lbk < 256 + 12) //Indirect
		{
			...
			if (oftp->mptr->INODE.i_block[12] == 0)
			{
				...
				put_block(oftp->mptr->dev, oftp->mptr->INODE.i_block[12], zero);
			}
			get_block(dev, oftp->mptr->INODE.i_block[12], indirect_buff);
		...
		else //Double indirect
		{
			...
			if(oftp->mptr->INODE.i_block[13] == 0)
			{
				...
				put_block(oftp->mptr->dev, oftp->mptr->INODE.i_block[13], zero);
				
			}
			get_block(dev, oftp->mptr->INODE.i_block[13], double_buff);

			lbk -= (12+256);
			
			if(double_buff[lbk /256]==0)
			{
				double_buff[lbk/256]=balloc(oftp->mptr->dev);
				put_block(oftp->mptr->dev, double_buff[lbk/256], zero);
			}
			
			dblk = double_buff[lbk/256];
			get_block(dev, dblk, double_buff);
